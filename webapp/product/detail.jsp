...
<%
	/*
		요청 URL
			localhost/comn/product/detail.jsp?no=xx
		요청파라미터
			no=xxx	상품번호
	*/
	...
	//			  staues=ProductStatus[no=0, name=상태명]
	Product product = prodcutDao.getProductByNo(no);
	
	// 이 상품과 관련된 이벤트목록조회
	List<Event> eventList = prodcutDao.getEventsByProductNo(no);
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<th>상태</th>
						<td><%=product.getStaus().getName() %></td>
					</tr>
					<tr>
						<th>등록일자</th>
