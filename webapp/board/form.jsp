...
<%
	/*
		로그인 여부를 체크해서 로그인되지 않은 사용자가 이 페이지를 이용할 수 없게 하기
			1. HttpSession에 저장된 사용자 정보를 조회한다.
			2. 로그인된 사용자 정보가 존재하지 않으면 로그인폼을 재요청하는 URL을 응답으로 보낸다.
				localhost/comn/loginform.jsp?error=deny
				
			* 상대경로 결정하기
				현재 URL = localhost/comn/board/form.jsp
				기준 URL = localhost/comn/board/
				
				목적 URL = localhost/comn/loginform.jsp
				상대 경로 = ../loginform.jsp
	*/
