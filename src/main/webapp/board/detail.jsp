...
<div class="container">
	<div class="row mb-3">
		<div class="col-12">
			<h1>게시글 상세</h1>
<%
	// HttpSession객체에 "LOGIN_USER"라는 속성명으로 저장된 인증된 상요자 정보조회
	LoginUser loginUser = (LoginUser) session.getAttribute("LOGIN_USER");
	/*
		요청 URL
			localhost/comn/board/detail.jsp?no=1000&page=3
		요청파라미터
			no=xxx&page=xxx
	...
	*/
	...
	// COMM_BOARDS 테이블에 대한 CURD 기능을 제공하는 BoardDao 객체 생성
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<th>수정일</th>
						<td><%= DateUtils.toText(board.getUpdateedDate()) %></td>
					</tr>
					<tr>
						<th>내용</th>
						<td colspan="3" >
							<%=StringUtils.withBr(board.getContent()) %>
						</td>
					</tr>
				</tbody>
			</table>
...
%>
					<a href="list.jsp?page=<%=currentPage %>" class="btn btn-primary">목록</a>
				</div>
	...
	<div class="row">
		<div class="col-12">
			...
			<form class="border bh-light p-3 mb-3" method="post" action="insertReply.jsp">
			...
			<input type="hidden" name="page" value="<%=currentPage %>">
				<div class="row mb-3">
					...
					<div class="col-sm-1 align-bottom">
...
%>
			<div class="card mb-3">
				<div class="card-heard py-1">
					<span><%=reply.getUser().getName() %></span>
					<small><%=DateUtils.toText(reply.getCreatedDate()) %></small>
					
					<a href="" class="btn btn-danger btn-sm float-end"><i class="bi bi-trash3"></i></a>
				</div>
